...
//                            }
                ...
                for (key,value) in viewRouter.mrzData {
                    if key == "passportNumber" {
                        passportDetails.passportNumber = "\(value)"
                    }
                    if key == "dateOfBirth" {
                        passportDetails.dateOfBirth = "\(value)"
                    }
                    if key == "expirationDate" {
                        passportDetails.expiryDate = "\(value)"
                    }
                }
            }
...
//        let pptData = "P<GBRTEST<<TEST<TEST<<<<<<<<<<<<<<<<<<<<<<<<1234567891GBR8001019M2106308<<<<<<<<<<<<<<04"
        ...
        return Group {
            NFCScanView().environment( \.colorScheme, .light).environmentObject(pd)
            NFCScanView().environment( \.colorScheme, .dark).environmentObject(pd)
        }
    }
}
#endif
